...
SCREEN::SCREEN(int _w, int _h, char transp)
{
	arr=0;
	t=transp;
	w=_w;
...
SCROLL::SCROLL(int _w, int _h, char transp) : SCREEN(_w,_h,transp)
{
	scroll=0;
}
...
BACKGROUND::BACKGROUND(int _w, int _h) : SCROLL(_w,_h,' ')
{
	Scroll(w);
}
...
LANDSCAPE::LANDSCAPE(int _w, int _h, const ASSET* _data) : SCROLL(_w,_h,'*')
{
	data = _data;
...
TERRAIN::TERRAIN(int _w, int _h, int _scrh, const char* sprite, const char* height, char* _hitbin, char _base_point, char _start_point, int _lives) : SCROLL(_w,_h,'*')
{
	hitbin = _hitbin;
	hits = 0;
...
void FNT::Paint(SCREEN* s, int x, int y, const char* txt)
{
	const char* ptr = txt;
	int dx = x - 1;
	while (*ptr)
...
LEVEL_MODAL::LEVEL_MODAL(SCREEN* _s, int _lives, int _start_lives, int* _score, int* _level_time, const char* _player_name,
			const char* _course_name, const LEVEL* _current_level, int start, char* _hitbin) :
	b(_s->w,16+_s->h-19-(_s->h-30)/2),
	m(_s->w,16,_current_level->bkgnd),
	d(_s->w,16,&dunes),
	t(_s->w,10,_s->h, _current_level->sprite, _current_level->height, _hitbin, _current_level->name + 1, _current_level->name + start, _lives)
{
	s=_s;
...
int LEVEL_MODAL::Run()
{
	...
	CON_INPUT exit_ir[4];
	int exit_irn=0;
	GetInputLen(&exit_irn);
	while (exit_irn)
	{
		ReadInput(exit_ir,4,&exit_irn);
		for (int i=0; i<exit_irn; i++)
		{
			if (exit_ir[i].EventType==CON_INPUT_KBD && exit_ir[i].Event.KeyEvent.bKeyDown)
			{
				if (ConfMapInput(exit_ir[i].Event.KeyEvent.uChar.AsciiChar)==27)
				{
					return -2;
				}
			}
