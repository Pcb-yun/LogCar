...
void Motor_View_Shell(void) {
	...
	while (1) {
		...
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "  ID | %3d   | %3d   | %3d   | %3d   |\r\n",
				g_motor_status[0]->motor_id,
				g_motor_status[1]->motor_id,
				g_motor_status[2]->motor_id,
				g_motor_status[3]->motor_id); line++;
#if MOTOR_STATUS_BUS_VOLTAGE
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "V(mV)|%7d|%7d|%7d|%7d|\r\n",
				g_motor_status[0]->voltage,
				g_motor_status[1]->voltage,
				g_motor_status[2]->voltage,
				g_motor_status[3]->voltage); line++;
#endif
#if MOTOR_STATUS_BUS_CURRENT
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "BusI |%7d|%7d|%7d|%7d|\r\n",
				g_motor_status[0]->bus_current,
				g_motor_status[1]->bus_current,
				g_motor_status[2]->bus_current,
				g_motor_status[3]->bus_current); line++;
#endif
#if MOTOR_STATUS_PHASE_CURRENT
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "PhI  |%7d|%7d|%7d|%7d|\r\n",
				g_motor_status[0]->phase_current,
				g_motor_status[1]->phase_current,
				g_motor_status[2]->phase_current,
				g_motor_status[3]->phase_current); line++;
#endif
#if MOTOR_STATUS_TEMPERATURE
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "Temp | %4d  | %4d  | %4d  | %4d  |\r\n",
				g_motor_status[0]->temp,
				g_motor_status[1]->temp,
				g_motor_status[2]->temp,
				g_motor_status[3]->temp); line++;
#endif
#if MOTOR_STATUS_BATTERY_VOLTAGE
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "BatV |%7d|%7d|%7d|%7d|\r\n",
				g_motor_status[0]->battery_voltage,
				g_motor_status[1]->battery_voltage,
				g_motor_status[2]->battery_voltage,
				g_motor_status[3]->battery_voltage); line++;
#endif
#if MOTOR_STATUS_SPEED
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "Vel  |%7d|%7d|%7d|%7d|\r\n",
				g_motor_status[0]->vel,
				g_motor_status[1]->vel,
				g_motor_status[2]->vel,
				g_motor_status[3]->vel); line++;
#endif
#if MOTOR_STATUS_REAL_POS
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "Pos  |%7s|%7s|%7s|%7s|\r\n",
				fmt_int(g_motor_status[0]->pos),
				fmt_int(g_motor_status[1]->pos),
				fmt_int(g_motor_status[2]->pos),
				fmt_int(g_motor_status[3]->pos)); line++;
#endif
#if MOTOR_STATUS_TARGET_POS
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "TPos |%7s|%7s|%7s|%7s|\r\n",
				fmt_int(g_motor_status[0]->target_pos),
				fmt_int(g_motor_status[1]->target_pos),
				fmt_int(g_motor_status[2]->target_pos),
				fmt_int(g_motor_status[3]->target_pos)); line++;
#endif
#if MOTOR_STATUS_POS_ERROR
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "Err  |%7s|%7s|%7s|%7s|\r\n",
				fmt_int(g_motor_status[0]->pos_error),
				fmt_int(g_motor_status[1]->pos_error),
				fmt_int(g_motor_status[2]->pos_error),
				fmt_int(g_motor_status[3]->pos_error)); line++;
#endif
#if MOTOR_STATUS_ENCODER_VALUE
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "EncL |%7d|%7d|%7d|%7d|\r\n",
				g_motor_status[0]->encoder_linear,
				g_motor_status[1]->encoder_linear,
				g_motor_status[2]->encoder_linear,
				g_motor_status[3]->encoder_linear); line++;
#endif
#if MOTOR_STATUS_ENCODER_RAW
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "EncR |%7d|%7d|%7d|%7d|\r\n",
				g_motor_status[0]->encoder_raw,
				g_motor_status[1]->encoder_raw,
				g_motor_status[2]->encoder_raw,
				g_motor_status[3]->encoder_raw); line++;
#endif
#if MOTOR_STATUS_MOTOR_FLAGS || MOTOR_STATUS_HOME_FLAGS
		{
			...
			for (int i = 0; i < 4; i++) {
#if MOTOR_STATUS_MOTOR_FLAGS
				sta[i] = (g_motor_status[i]->oac << 7) | (g_motor_status[i]->esi_r << 6) |
						 (g_motor_status[i]->esi_l << 4) | (g_motor_status[i]->cgp << 3) |
						 (g_motor_status[i]->cgi << 2) | (g_motor_status[i]->prf << 1) |
						 g_motor_status[i]->ens;
#else
				(void) sta[i];
#endif
#if MOTOR_STATUS_HOME_FLAGS
				hom[i] = (g_motor_status[i]->ocp_tf << 7) | (g_motor_status[i]->otp_tf << 4) |
						 (g_motor_status[i]->org_cf << 3) | (g_motor_status[i]->org_sf << 2) |
						 (g_motor_status[i]->cal_rdy << 1) | g_motor_status[i]->enc_rdy;
#else
				(void) hom[i];
#endif
			}
		...
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "Pin  |  %04X |  %04X |  %04X |  %04X |\r\n",
				g_motor_status[0]->pin_status,
				g_motor_status[1]->pin_status,
				g_motor_status[2]->pin_status,
				g_motor_status[3]->pin_status); line++;
#endif
#if MOTOR_STATUS_SET_POS
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "SPos |%7s|%7s|%7s|%7s|\r\n",
				fmt_int(g_motor_status[0]->set_pos),
				fmt_int(g_motor_status[1]->set_pos),
				fmt_int(g_motor_status[2]->set_pos),
				fmt_int(g_motor_status[3]->set_pos)); line++;
#endif
#if MOTOR_STATUS_INPUT_PULSES
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "Puls |%7s|%7s|%7s|%7s|\r\n",
				fmt_int(g_motor_status[0]->input_pulses),
				fmt_int(g_motor_status[1]->input_pulses),
				fmt_int(g_motor_status[2]->input_pulses),
				fmt_int(g_motor_status[3]->input_pulses)); line++;
#endif
#if MOTOR_READ_VERSION
		{
			static char fw[4][8];
			for (int i = 0; i < 4; i++) {
				uint16_t v = g_motor_status[i]->firmware_version;
		...
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "HWVer|  %4d  |  %4d  |  %4d  |  %4d  |\r\n",
				g_motor_status[0]->hardware_version,
				g_motor_status[1]->hardware_version,
				g_motor_status[2]->hardware_version,
				g_motor_status[3]->hardware_version); line++;
		{
			static char hw_type[4][8];
			for (int i = 0; i < 4; i++) {
				uint8_t series = g_motor_status[i]->hardware_series;
				uint8_t type = g_motor_status[i]->hardware_type;
		...
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "LockL|  %4d  |  %4d  |  %4d  |  %4d  |\r\n",
				g_motor_status[0]->lock_level,
				g_motor_status[1]->lock_level,
				g_motor_status[2]->lock_level,
				g_motor_status[3]->lock_level); line++;
#endif
#if MOTOR_FIRMWARE_TYPE
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "FwTyp|  %-4s |  %-4s |  %-4s |  %-4s |\r\n",
				g_motor_status[0]->firmware_type ? "Emm" : "X",
				g_motor_status[1]->firmware_type ? "Emm" : "X",
				g_motor_status[2]->firmware_type ? "Emm" : "X",
				g_motor_status[3]->firmware_type ? "Emm" : "X"); line++;
#endif
#if MOTOR_DIRECTION
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "MotDr|  %-4s |  %-4s |  %-4s |  %-4s |\r\n",
				g_motor_status[0]->motor_dir ? "CCW" : "CW",
				g_motor_status[1]->motor_dir ? "CCW" : "CW",
				g_motor_status[2]->motor_dir ? "CCW" : "CW",
				g_motor_status[3]->motor_dir ? "CCW" : "CW"); line++;
#endif
#if MOTOR_PID_READ
		...
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "Kp   |%7d|%7d|%7d|%7d|\r\n",
				...
				g_motor_status[1]->kp,
				g_motor_status[2]->kp,
				g_motor_status[3]->kp); line++;
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "Ki   |%7d|%7d|%7d|%7d|\r\n",
				g_motor_status[0]->ki,
				g_motor_status[1]->ki,
				g_motor_status[2]->ki,
				g_motor_status[3]->ki); line++;
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "Kd   |%7d|%7d|%7d|%7d|\r\n",
				g_motor_status[0]->kd,
				g_motor_status[1]->kd,
				g_motor_status[2]->kd,
				g_motor_status[3]->kd); line++;
#elif CURRENT_FIRMWARE == FIRMWARE_X
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "TrKp |%7d|%7d|%7d|%7d|\r\n",
				g_motor_status[0]->trapezoidal_kp,
				g_motor_status[1]->trapezoidal_kp,
				g_motor_status[2]->trapezoidal_kp,
				g_motor_status[3]->trapezoidal_kp); line++;
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "DiKp |%7d|%7d|%7d|%7d|\r\n",
				g_motor_status[0]->direct_kp,
				g_motor_status[1]->direct_kp,
				g_motor_status[2]->direct_kp,
				g_motor_status[3]->direct_kp); line++;
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "VelKp|%7d|%7d|%7d|%7d|\r\n",
				g_motor_status[0]->vel_kp,
				g_motor_status[1]->vel_kp,
				g_motor_status[2]->vel_kp,
				g_motor_status[3]->vel_kp); line++;
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "VelKi|%7d|%7d|%7d|%7d|\r\n",
				g_motor_status[0]->vel_ki,
				g_motor_status[1]->vel_ki,
				g_motor_status[2]->vel_ki,
				g_motor_status[3]->vel_ki); line++;
#endif
#endif
#if MOTOR_DRIVER_POS_WINDOW
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "PosW |%7d|%7d|%7d|%7d|\r\n",
				g_motor_status[0]->pos_window,
				g_motor_status[1]->pos_window,
				g_motor_status[2]->pos_window,
				g_motor_status[3]->pos_window); line++;
#endif
#if MOTOR_INTEGRAL_LIMIT_READ
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "IntL |%7d|%7d|%7d|%7d|\r\n",
				g_motor_status[0]->integral_limit,
				g_motor_status[1]->integral_limit,
				g_motor_status[2]->integral_limit,
				g_motor_status[3]->integral_limit); line++;
#endif
#if MOTOR_PROTECT_THRESHOLD_READ
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "TempT|%7d|%7d|%7d|%7d|\r\n",
				g_motor_status[0]->temp_threshold,
				g_motor_status[1]->temp_threshold,
				g_motor_status[2]->temp_threshold,
				g_motor_status[3]->temp_threshold); line++;
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "CurrT|%7d|%7d|%7d|%7d|\r\n",
				g_motor_status[0]->current_threshold,
				g_motor_status[1]->current_threshold,
				g_motor_status[2]->current_threshold,
				g_motor_status[3]->current_threshold); line++;
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "ProtT|%7d|%7d|%7d|%7d|\r\n",
				g_motor_status[0]->protect_time,
				g_motor_status[1]->protect_time,
				g_motor_status[2]->protect_time,
				g_motor_status[3]->protect_time); line++;
#endif
#if MOTOR_HEARTBEAT_READ
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "HearT|%7d|%7d|%7d|%7d|\r\n",
				g_motor_status[0]->heartbeat_time,
				g_motor_status[1]->heartbeat_time,
				g_motor_status[2]->heartbeat_time,
				g_motor_status[3]->heartbeat_time); line++;
#endif
#if MOTOR_COLLISION_ANGLE_READ
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "ColA |%7d|%7d|%7d|%7d|\r\n",
				g_motor_status[0]->collision_angle,
				g_motor_status[1]->collision_angle,
				g_motor_status[2]->collision_angle,
				g_motor_status[3]->collision_angle); line++;
#endif
#if MOTOR_DRIVER_STALL_PROTECT
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "ClogE| %-5s | %-5s | %-5s | %-5s |\r\n",
				onoff_str(g_motor_status[0]->clog_enable),
				onoff_str(g_motor_status[1]->clog_enable),
				onoff_str(g_motor_status[2]->clog_enable),
				onoff_str(g_motor_status[3]->clog_enable)); line++;
#endif
#if MOTOR_DRIVER_STALL_SPEED
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "ClogR|%7d|%7d|%7d|%7d|\r\n",
				g_motor_status[0]->clog_rpm,
				g_motor_status[1]->clog_rpm,
				g_motor_status[2]->clog_rpm,
				g_motor_status[3]->clog_rpm); line++;
#endif
#if MOTOR_DRIVER_STALL_CURRENT
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "ClogC|%7d|%7d|%7d|%7d|\r\n",
				g_motor_status[0]->clog_current,
				g_motor_status[1]->clog_current,
				g_motor_status[2]->clog_current,
				g_motor_status[3]->clog_current); line++;
#endif
#if MOTOR_DRIVER_STALL_TIME
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "ClogT|%7d|%7d|%7d|%7d|\r\n",
				g_motor_status[0]->clog_time,
				g_motor_status[1]->clog_time,
				g_motor_status[2]->clog_time,
				g_motor_status[3]->clog_time); line++;
#endif
#if MOTOR_DRIVER_HOME
#if MOTOR_DRIVER_HOME_MODE
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "HomeM| %-4s  | %-4s  | %-4s  | %-4s  |\r\n",
				home_mode_str(g_motor_status[0]->home_mode),
				home_mode_str(g_motor_status[1]->home_mode),
				home_mode_str(g_motor_status[2]->home_mode),
				home_mode_str(g_motor_status[3]->home_mode)); line++;
#endif
#if MOTOR_DRIVER_HOME_DIR
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "HomeD| %-4s | %-4s | %-4s | %-4s |\r\n",
				dir_str(g_motor_status[0]->home_dir),
				dir_str(g_motor_status[1]->home_dir),
				dir_str(g_motor_status[2]->home_dir),
				dir_str(g_motor_status[3]->home_dir)); line++;
#endif
#if MOTOR_DRIVER_HOME_SPEED
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "HomeS|%7d|%7d|%7d|%7d|\r\n",
				g_motor_status[0]->home_speed,
				g_motor_status[1]->home_speed,
				g_motor_status[2]->home_speed,
				g_motor_status[3]->home_speed); line++;
#endif
#if MOTOR_DRIVER_HOME_TIMEOUT
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "HomeT|%7d|%7d|%7d|%7d|\r\n",
				g_motor_status[0]->home_timeout,
				g_motor_status[1]->home_timeout,
				g_motor_status[2]->home_timeout,
				g_motor_status[3]->home_timeout); line++;
#endif
#if MOTOR_DRIVER_AUTO_HOME
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "HmAEn| %-4s  | %-4s  | %-4s  | %-4s  |\r\n",
				onoff_str(g_motor_status[0]->auto_home),
				onoff_str(g_motor_status[1]->auto_home),
				onoff_str(g_motor_status[2]->auto_home),
				onoff_str(g_motor_status[3]->auto_home)); line++;
#endif
#if MOTOR_DRIVER_HOME_COLLISION_SPEED
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "SlVel|%7d|%7d|%7d|%7d|\r\n",
				g_motor_status[0]->sl_vel,
				g_motor_status[1]->sl_vel,
				g_motor_status[2]->sl_vel,
				g_motor_status[3]->sl_vel); line++;
#endif
#if MOTOR_DRIVER_HOME_COLLISION_CURRENT
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "SlCur|%7d|%7d|%7d|%7d|\r\n",
				g_motor_status[0]->sl_current,
				g_motor_status[1]->sl_current,
				g_motor_status[2]->sl_current,
				g_motor_status[3]->sl_current); line++;
#endif
#if MOTOR_DRIVER_HOME_COLLISION_TIME
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "SlTim|%7d|%7d|%7d|%7d|\r\n",
				g_motor_status[0]->sl_time,
				g_motor_status[1]->sl_time,
				g_motor_status[2]->sl_time,
				g_motor_status[3]->sl_time); line++;
#endif
#endif /* MOTOR_DRIVER_HOME */

#if MOTOR_DRIVER_CONTROL_MODE
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "CtrlM| %-4s  | %-4s  | %-4s  | %-4s  |\r\n",
				ctrl_str(g_motor_status[0]->control_mode),
				ctrl_str(g_motor_status[1]->control_mode),
				ctrl_str(g_motor_status[2]->control_mode),
				ctrl_str(g_motor_status[3]->control_mode));
		line++;
#endif

#if MOTOR_DRIVER_MOTOR_TYPE
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "MotTp| %-4s | %-4s | %-4s | %-4s |\r\n",
				motor_type_str(g_motor_status[0]->motor_type),
				motor_type_str(g_motor_status[1]->motor_type),
				motor_type_str(g_motor_status[2]->motor_type),
				motor_type_str(g_motor_status[3]->motor_type));
		line++;
#endif
...
#if MOTOR_DRIVER_MICRO_STEP
		{
			static char ms[4][6];
			for (int i = 0; i < 4; i++) {
				uint8_t v = g_motor_status[i]->micro_step;
		...
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "Inter| %-4s | %-4s | %-4s | %-4s |\r\n",
				onoff_str(g_motor_status[0]->interpolation),
				onoff_str(g_motor_status[1]->interpolation),
				onoff_str(g_motor_status[2]->interpolation),
				onoff_str(g_motor_status[3]->interpolation));
		line++;
#endif

#if MOTOR_DRIVER_OPENLOOP_CURRENT
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "OpenI|%7d|%7d|%7d|%7d|\r\n",
				g_motor_status[0]->open_current,
				g_motor_status[1]->open_current,
				g_motor_status[2]->open_current,
				g_motor_status[3]->open_current);
		line++;
#endif

#if MOTOR_DRIVER_CLOSEDLOOP_CURRENT
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "ClosI|%7d|%7d|%7d|%7d|\r\n",
				g_motor_status[0]->close_current,
				g_motor_status[1]->close_current,
				g_motor_status[2]->close_current,
				g_motor_status[3]->close_current);
		line++;
#endif

#if MOTOR_DRIVER_BAUDRATE
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "Uart | %-6s | %-6s | %-6s | %-6s |\r\n",
				uart_baud_str(g_motor_status[0]->uart_baudrate),
				uart_baud_str(g_motor_status[1]->uart_baudrate),
				uart_baud_str(g_motor_status[2]->uart_baudrate),
				uart_baud_str(g_motor_status[3]->uart_baudrate));
		line++;
#endif

#if MOTOR_DRIVER_CAN_RATE
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "Can  | %-6s | %-6s | %-6s | %-6s |\r\n",
				can_baud_str(g_motor_status[0]->can_baudrate),
				can_baud_str(g_motor_status[1]->can_baudrate),
				can_baud_str(g_motor_status[2]->can_baudrate),
				can_baud_str(g_motor_status[3]->can_baudrate));
		line++;
#endif

#if MOTOR_DRIVER_LOCK_KEY
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "LockK| %-4s | %-4s | %-4s | %-4s |\r\n",
				onoff_str(g_motor_status[0]->lock_key),
				onoff_str(g_motor_status[1]->lock_key),
				onoff_str(g_motor_status[2]->lock_key),
				onoff_str(g_motor_status[3]->lock_key));
		line++;
#endif

#if MOTOR_DRIVER_PULSE_PORT_MODE
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "PulsP| %-5s| %-5s| %-5s| %-5s|\r\n",
				pulse_port_str(g_motor_status[0]->pulse_port_mode),
				pulse_port_str(g_motor_status[1]->pulse_port_mode),
				pulse_port_str(g_motor_status[2]->pulse_port_mode),
				pulse_port_str(g_motor_status[3]->pulse_port_mode));
		line++;
#endif

#if MOTOR_DRIVER_COMM_PORT_MODE
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "CommP| %-5s| %-5s| %-5s| %-5s|\r\n",
				comm_port_str(g_motor_status[0]->comm_port_mode),
				comm_port_str(g_motor_status[1]->comm_port_mode),
				comm_port_str(g_motor_status[2]->comm_port_mode),
				comm_port_str(g_motor_status[3]->comm_port_mode));
		line++;
#endif

#if MOTOR_DRIVER_EN_PIN_LEVEL
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "EnPin| %-5s| %-5s| %-5s| %-5s|\r\n",
				en_pin_str(g_motor_status[0]->en_pin_level),
				en_pin_str(g_motor_status[1]->en_pin_level),
				en_pin_str(g_motor_status[2]->en_pin_level),
				en_pin_str(g_motor_status[3]->en_pin_level));
		line++;
#endif

#if MOTOR_DRIVER_DIR_PIN_LEVEL
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "DirPi| %-5s| %-5s| %-5s| %-5s|\r\n",
				dir_pin_str(g_motor_status[0]->dir_pin_level),
				dir_pin_str(g_motor_status[1]->dir_pin_level),
				dir_pin_str(g_motor_status[2]->dir_pin_level),
				dir_pin_str(g_motor_status[3]->dir_pin_level));
		line++;
#endif

#if MOTOR_DRIVER_AUTO_SCREEN_OFF
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "ScrOf| %-4s | %-4s | %-4s | %-4s |\r\n",
				onoff_str(g_motor_status[0]->auto_screen_off),
				onoff_str(g_motor_status[1]->auto_screen_off),
				onoff_str(g_motor_status[2]->auto_screen_off),
				onoff_str(g_motor_status[3]->auto_screen_off));
		line++;
#endif

#if MOTOR_DRIVER_COMM_CHECK_MODE
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "ChkMd| %-5s| %-5s| %-5s| %-5s|\r\n",
				check_mode_str(g_motor_status[0]->comm_check_mode),
				check_mode_str(g_motor_status[1]->comm_check_mode),
				check_mode_str(g_motor_status[2]->comm_check_mode),
				check_mode_str(g_motor_status[3]->comm_check_mode));
		line++;
#endif

#if MOTOR_DRIVER_CMD_RESPONSE_MODE
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "RspMd| %-5s| %-5s| %-5s| %-5s|\r\n",
				rsp_mode_str(g_motor_status[0]->cmd_response_mode),
				rsp_mode_str(g_motor_status[1]->cmd_response_mode),
				rsp_mode_str(g_motor_status[2]->cmd_response_mode),
				rsp_mode_str(g_motor_status[3]->cmd_response_mode));
		line++;
#endif

#if MOTOR_DRIVER_CLOSEDLOOP_MAX_SPEED
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "ClSpd|%7d|%7d|%7d|%7d|\r\n",
				g_motor_status[0]->close_max_speed,
				g_motor_status[1]->close_max_speed,
				g_motor_status[2]->close_max_speed,
				g_motor_status[3]->close_max_speed);
		line++;
#endif

#if MOTOR_DRIVER_CURRENT_LOOP_BANDWIDTH
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "CurBW|%7d|%7d|%7d|%7d|\r\n",
				g_motor_status[0]->current_loop_bw,
				g_motor_status[1]->current_loop_bw,
				g_motor_status[2]->current_loop_bw,
				g_motor_status[3]->current_loop_bw);
		line++;
#endif

#if MOTOR_DRIVER_SCALE_10X
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "Spd10| %-4s | %-4s | %-4s | %-4s |\r\n",
				onoff_str(g_motor_status[0]->scale_10x),
				onoff_str(g_motor_status[1]->scale_10x),
				onoff_str(g_motor_status[2]->scale_10x),
				onoff_str(g_motor_status[3]->scale_10x));
		line++;
#endif

#if MOTOR_DRIVER_CLOSEDLOOP_MAX_VOLTAGE
		len += snprintf(out_buf + len, BUFFER_SIZE - len, "MaxVo|%7d|%7d|%7d|%7d|\r\n",
				g_motor_status[0]->close_max_voltage,
				g_motor_status[1]->close_max_voltage,
				g_motor_status[2]->close_max_voltage,
				g_motor_status[3]->close_max_voltage);
		line++;
#endif
