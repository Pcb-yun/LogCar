...
void Motor_Receive(uint8_t *data, uint8_t len) {
	...
	if (motor_id > 4 || motor_id == 0) return;
	...
	switch (cmd_code) {
        ...
        case CMD_SET_LOCK_PARAMS: case CMD_SET_DRIVER_CONFIG: case CMD_RESET_CURPOS_TO_ZERO:
            if (len >= 4) {
                ...
                if (response_code == 0xE2) { logWarning("Motor %d parameter error", motor_id); }
                else if (response_code == 0xEE)  { logWarning("Motor %d command format error", motor_id); }
            } break;
	    /* ---- 系统状态参数读取应答 ---- */
	    ...
	    /* ---- 驱动配置参数读取应答 ---- */
#if MOTOR_HOME_READ
        case CMD_READ_HOME_PARAMS: if (len >= 17) { motor->home_mode = data[2]; motor->home_dir = data[3]; motor->home_speed = (data[4] << 8) | data[5]; motor->home_timeout = ((uint32_t)data[6] << 24) | ((uint32_t)data[7] << 16) | ((uint32_t)data[8] << 8) | data[9];
#if MOTOR_DRIVER_HOME_SPEED
                motor->sl_vel = (data[10] << 8) | data[11];
#endif /* MOTOR_DRIVER_HOME_SPEED */
#if MOTOR_DRIVER_HOME_DIR
                motor->sl_current = (data[12] << 8) | data[13];
#endif /* MOTOR_DRIVER_HOME_DIR */
#if MOTOR_DRIVER_HOME_TIMEOUT
                motor->sl_time = (data[14] << 8) | data[15];
#endif /* MOTOR_DRIVER_HOME_TIMEOUT */
#if MOTOR_DRIVER_HOME_MODE
                motor->home_auto_enable = data[16];
#endif /* MOTOR_DRIVER_HOME_MODE */
		} break;
#endif /* MOTOR_HOME_READ */
        /* ---- 控制参数读取应答 ---- */
#if MOTOR_PID_READ
	case CMD_READ_PID_PARAMS:
...
#endif /* MOTOR_DRIVER_INTEGRAL_LIMIT */
        ...
        case CMD_READ_VERSION_INFO: if (len >= 6) { motor->firmware_version = (data[2] << 8) | data[3]; motor->hardware_version = data[5]; } break;
        ...
        case CMD_READ_OPTION_PARAMS: if (len >= 5) { motor->option_params = data[2]; motor->lock_level = data[3] & 0x03; } break;
...
#endif /* MOTOR_STATUS_ENCODER_VALUE */
#if MOTOR_STATUS_TARGET_POS
                { int32_t tp = ((int32_t)data[9] << 24) | ((int32_t)data[10] << 16) | ((int32_t)data[11] << 8) | (int32_t)data[12]; if (data[8] == 0x01) tp = -tp; motor->target_pos = tp; }
#endif /* MOTOR_STATUS_TARGET_POS */
#if MOTOR_STATUS_SPEED
			    { int16_t spd = ((int16_t)data[14] << 8) | (int16_t)data[15]; if (data[13] == 0x01) spd = -spd; motor->vel = spd; }
#endif /* MOTOR_STATUS_SPEED */
#if MOTOR_STATUS_REAL_POS
			    { int32_t p = ((int32_t)data[17] << 24) | ((int32_t)data[18] << 16) | ((int32_t)data[19] << 8) | (int32_t)data[20]; if (data[16] == 0x01) p = -p; motor->pos = p; }
#endif /* MOTOR_STATUS_REAL_POS */
#if MOTOR_STATUS_POS_ERROR
			    { int32_t e = ((int32_t)data[22] << 24) | ((int32_t)data[23] << 16) | ((int32_t)data[24] << 8) | (int32_t)data[25]; if (data[21] == 0x01) e = -e; motor->pos_error = e; }
#endif /* MOTOR_STATUS_POS_ERROR */
#if MOTOR_STATUS_TEMPERATURE
			    motor->temp = (data[26] == 0x01) ? -(int8_t)data[27] : (int8_t)data[27];
#endif /* MOTOR_STATUS_TEMPERATURE */
#if MOTOR_STATUS_MOTOR_FLAGS
			    { uint8_t st = (len >= 32) ? data[29] : data[30]; motor->ens = (st & 0x01) != 0; motor->prf = (st & 0x02) != 0; motor->cgi = (st & 0x04) != 0; motor->cgp = (st & 0x08) != 0; motor->esi_l = (st & 0x10) != 0; motor->esi_r = (st & 0x40) != 0; motor->oac = (st & 0x80) != 0; }
#endif /* MOTOR_STATUS_MOTOR_FLAGS */
            }
#elif CURRENT_FIRMWARE == FIRMWARE_X
		    if (len >= 37) {
...
#endif /* MOTOR_STATUS_ENCODER_VALUE */
#if MOTOR_STATUS_TARGET_POS
			    { int32_t tp = ((int32_t)data[13] << 24) | ((int32_t)data[14] << 16) | ((int32_t)data[15] << 8) | (int32_t)data[16]; if (data[12] == 0x01) tp = -tp; motor->target_pos = tp; }
#endif /* MOTOR_STATUS_TARGET_POS */
#if MOTOR_STATUS_SPEED
			    { int16_t spd = ((int16_t)data[18] << 8) | (int16_t)data[19]; if (data[17] == 0x01) spd = -spd; motor->vel = spd; }
#endif /* MOTOR_STATUS_SPEED */
#if MOTOR_STATUS_REAL_POS
			    { int32_t p = ((int32_t)data[21] << 24) | ((int32_t)data[22] << 16) | ((int32_t)data[23] << 8) | (int32_t)data[24]; if (data[20] == 0x01) p = -p; motor->pos = p; }
#endif /* MOTOR_STATUS_REAL_POS */
#if MOTOR_STATUS_POS_ERROR
                { int32_t e = ((int32_t)data[26] << 24) | ((int32_t)data[27] << 16) | ((int32_t)data[28] << 8) | (int32_t)data[29]; if (data[25] == 0x01) e = -e; motor->pos_error = e; }
#endif /* MOTOR_STATUS_POS_ERROR */
#if MOTOR_STATUS_TEMPERATURE
			    motor->temp = (data[30] == 0x01) ? -(int8_t)data[31] : (int8_t)data[31];
#endif /* MOTOR_STATUS_TEMPERATURE */
#if MOTOR_STATUS_MOTOR_FLAGS
			    { uint8_t st = data[33]; motor->ens = (st & 0x01) != 0; motor->prf = (st & 0x02) != 0; motor->cgi = (st & 0x04) != 0; motor->cgp = (st & 0x08) != 0; motor->esi_l = (st & 0x10) != 0; motor->esi_r = (st & 0x40) != 0; motor->oac = (st & 0x80) != 0; }
#endif /* MOTOR_STATUS_MOTOR_FLAGS */
		    }
#endif /* CURRENT_FIRMWARE */
		break;
...
#endif /* MOTOR_DRIVER_CLOSEDLOOP_CURRENT */
#if MOTOR_DRIVER_POWER_FLAG
			    motor->power_flag    = data[17];
#endif /* MOTOR_DRIVER_POWER_FLAG */
#if MOTOR_DRIVER_DIRECTION
			    motor->motor_direction = data[19];
#endif /* MOTOR_DRIVER_DIRECTION */
#if MOTOR_DRIVER_BAUDRATE
			    motor->uart_baudrate = data[21];
#endif /* MOTOR_DRIVER_BAUDRATE */
#if MOTOR_DRIVER_CAN_RATE
			    motor->can_baudrate  = data[22];
#endif /* MOTOR_DRIVER_CAN_RATE */
#if MOTOR_DRIVER_POS_ARRIVE
			    motor->pos_arrive    = data[23];
#endif /* MOTOR_DRIVER_POS_ARRIVE */
#if MOTOR_DRIVER_PULSE_THRESHOLD
			    motor->pulse_threshold = ((uint16_t)data[24] << 8) | (uint16_t)data[25];
#endif /* MOTOR_DRIVER_PULSE_THRESHOLD */
#if MOTOR_DRIVER_STALL_PROTECT
...
static void Motor_Process_Ctrl(uint8_t motor_id, MotorCtrl_t *ctrl) {
	switch (ctrl->type) {
	    ...
	    case CTRL_POS_DIRECT: if (ctrl->p.pos_dir.max_current > 0) {
			    ZDT_V5_Pos_Control_Direct_With_Limit(motor_id, ctrl->p.pos_dir.dir, ctrl->p.pos_dir.vel, (uint32_t)ctrl->p.pos_dir.target, ctrl->p.pos_dir.mode, ctrl->p.pos_dir.snF, ctrl->p.pos_dir.max_current);
		    } else {
			    ZDT_V5_Pos_Control_Direct(motor_id, ctrl->p.pos_dir.dir, ctrl->p.pos_dir.vel, (uint32_t)ctrl->p.pos_dir.target, ctrl->p.pos_dir.mode, ctrl->p.pos_dir.snF); } break;
#endif /* MOTOR_POS_MODE_DIRECT */
#if MOTOR_TORQUE_MODE
	    case CTRL_TORQUE: if (ctrl->p.torque.max_vel > 0) {
			    ZDT_V5_Torque_Control_With_Limit(motor_id, ctrl->p.torque.dir, ctrl->p.torque.slope, ctrl->p.torque.current, ctrl->p.torque.max_vel, ctrl->p.torque.sync);
		    } else {
			    ZDT_V5_Torque_Control(motor_id, ctrl->p.torque.dir, ctrl->p.torque.slope, ctrl->p.torque.current, ctrl->p.torque.sync); } break;
#endif /* MOTOR_TORQUE_MODE */
#if MOTOR_CMD_STOP
	    case CTRL_STOP: ZDT_V5_Stop(motor_id, ctrl->p.stop.sync); break;
#endif
#if MOTOR_SYNC_TRIGGER
	    case CTRL_SYNC: ZDT_V5_Synchronous_Motion(0); break;
#endif
#if MOTOR_MULTI_CMD /* 多电机命令需额外数据，由应用层处理 */
        case CTRL_MULTI: logWarning("CTRL_MULTI not directly supported, use dedicated API"); break;
#endif
	...
	case CTRL_FAST_SEND: ZDT_V5_Fast_Send_Pos(motor_id, ctrl->p.fast_send.pos); break;
        ...
        case CTRL_NONE: default: logWarning("Unknown or none control type: %d", ctrl->type); break;
	}
}
...
#endif
        case TRIG_NONE: default: logWarning("Unknown or none trigger type: %d", trigger->type); break;
	}
}
...
static void Motor_Process_Param_Read(uint8_t motor_id, MotorParamRead_t *read) {
	switch (read->type) {
	    ...
	    case PARAM_BATCH: ZDT_V5_Read_All_Sys_Params(motor_id); break;
#endif
#if MOTOR_BROADCAST_READ_ID
	    case PARAM_MOTOR_ID: ZDT_V5_Read_Device_Info_Params(motor_id, I_ID); break;
#endif
#if MOTOR_DRIVER_CONFIG_READ_BATCH
	    case PARAM_DRIVER_CONFIG_ALL: ZDT_V5_Read_Batch_Config(motor_id); break;
#endif
#if MOTOR_PID_READ
        case PARAM_PID: ZDT_V5_Read_Ctrl_Params(motor_id, C_PID); break;
#endif
#if MOTOR_HOME_READ
	    case PARAM_HOME_PARAMS: ZDT_V5_Read_Ctrl_Params(motor_id, C_HOME); break;
#endif
#if MOTOR_PROTECT_THRESHOLD_READ
	    case PARAM_PROTECT_THRESHOLD: ZDT_V5_Read_Ctrl_Params(motor_id, C_PROTECT_THRESHOLD); break;
#endif
#if MOTOR_COLLISION_ANGLE_READ
	    case PARAM_COLLISION_ANGLE: ZDT_V5_Read_Ctrl_Params(motor_id, C_COLLISION_ANGLE); break;
#endif
#if MOTOR_HEARTBEAT_READ
	    case PARAM_HEARTBEAT: ZDT_V5_Read_Ctrl_Params(motor_id, C_HEARTBEAT); break;
#endif
#if MOTOR_DRIVER_INTEGRAL_LIMIT
	    case PARAM_INTEGRAL_LIMIT: ZDT_V5_Read_Ctrl_Params(motor_id, C_INTEGRAL_LIMIT); break;
#endif
#if MOTOR_DRIVER_POS_WINDOW
	    case PARAM_POS_WINDOW: ZDT_V5_Read_Driver_Params(motor_id, D_POS_WINDOW); break;
#endif
#if MOTOR_READ_VERSION
	    case PARAM_FIRMWARE_TYPE: ZDT_V5_Read_Device_Info_Params(motor_id, I_VERSION); break;
#endif
#if MOTOR_READ_PHASE_PARAMS
	    case PARAM_MOTOR_TYPE: ZDT_V5_Read_Device_Info_Params(motor_id, I_PHASE_PARAMS); break;
#endif
#if MOTOR_READ_OPTION_PARAMS
	    case PARAM_LOCK_PARAMS: ZDT_V5_Read_Device_Info_Params(motor_id, I_OPTION); break;
#endif
#if MOTOR_DRIVER_DMX512
	    case PARAM_DMX512: ZDT_V5_Read_Device_Info_Params(motor_id, I_DMX512); break;
#endif
        case PARAM_NONE: default: logWarning("Unknown or none param read type: %d", read->type); break;
	}
}
...
#endif
        case PARAM_NONE: default: logWarning("Unknown or none param write type: %d", write->type); break;
	}
}
